...
public interface KnowledgeIndexSnapshotMapper {
    ...
    @Insert("""
            INSERT INTO rag_index_snapshots(tenant_id, kb_id, snapshot_id, status)
            VALUES (1, #{kbId}, #{snapshotId}, 'REGISTERED')
            ON CONFLICT (tenant_id, kb_id, snapshot_id) DO NOTHING
            """)
