...
### Step 8 · 客户端 core-meter 镜像 + 顶栏余额环（FR-041，AC-045 客户端半边）✅（meter_sync userId 暂 0，P04 登录态注入）
...
### Step 9 · 云端 CI + 收尾（FR-041/050 交付面）✅（CI 首推观察；基准已记录）
...
- [x] BCrypt 密码 + 验证码限频（Redis）（密码登录预留，验证码限频 e2e 锁定）
- [x] JWT 15min/7d + 黑名单登出（e2e 锁定）
- [x] 全部写接口幂等（idempotency_key UNIQUE 兜底，charge/recharge/search/镜像均测）
- [x] 钱包乐观锁 + 事务；账本只增不改（100 并发不超扣 e2e）
- [x] 回调验签（HMAC）；金额全 `*_cents` 整数（伪造签名/篡改金额 e2e 400）
- [x] 上游 Key AES-GCM 加密 + 日志脱敏 + 客户端不可见（crypto.spec 锁定；真上游配 key 时生效）
- [x] 体验金双因子防薅（deviceId 唯一拦截）；单任务消费上限熔断（CHAT_TASK_CAP_CENTS，done 帧 capped=true）
- [x] 审计：账本即审计（只增）+ BillingService.credit kind=4 管理调整入口已留（管理后台 P08）
