...
## Step 2 · core-exec：命令执行器（FR-003 地基）✅（cargo test 5/5；fmt/clippy 0 错）
...
## Step 3 · 两档审批模式（FR-009）✅（core-sandbox approval 6 单测 + core-state 审批表 1 单测 + Tauri 命令）
...
## Step 4 · 环境探测与画像缓存（FR-005 / AC-005）✅（core-exec probe/profile 9 单测 + L5 env_profiles + cache hit）
...
## Step 5 · 一键安装向导（FR-005 / AC-006）✅（install.rs 计划生成/执行 + InstallWizard UI + Tauri 命令；core-exec 12 单测）
...
## Step 6 · 全流程闭环 v1（FR-003 / AC-004）✅（runner.rs 文件读写/安装/测试/修复/commit + Tauri 命令 + TaskRunnerPanel；core-exec 17 单测）
...
## Step 7 · Secrets 管理（FR-012 / AC-014）✅（L6 secrets 表 + keyring/AES-GCM 加密 + runner 注入/脱敏 + SecretsPanel UI；core-state 3 单测 + core-exec 3 单测）

- **目标**：用户配的密钥只进进程环境变量，UI/日志/快照里全是 `***`。
- **动作**：
  1. core-state 新增 `secrets` 表（L6__secrets.sql；id/project_id/name/encrypted_value/created_at）。
...
## Step 8 · 状态机集成：阶段 3 chunk 触发执行（FR-003 + 状态机联动）✅（execute_build 命令 + BuildButton UI；建造阶段自动跑 runner→成功推进验收/失败留在建造）
...
## Step 9 · 冒烟、文档、CI 与归档 ✅（Feature Map / User-Ops / README / db_schema / 00_MVP索引更新；check_all + check_docs 全绿）
