...
- [ ] **Step 1：F-1 修复①② 下载端点 inline 白名单 + nosniff**（最高优先，已确认可利用 🔴）
...
- [ ] **Step 2：F-1 修复③ 上传扩展名白名单** `[依赖: Step 1]`
...
- [ ] **Step 3：A1 登录防爆破（复用现有 audit_logs）** `[P] 可并行`
  ...
  - **需人工介入**：否
  - **安全检查**：A07、A09
  - **验证**（渗透自查·认证）：登录 1 分钟 50 次 → 429/锁定；同账号错 5 次锁 15min，到点自动解锁；停 Redis 降级放行且 WARN；audit_logs 有锁定事件行。

- [ ] **Step 4：A2 安全响应头 + A3 CORS 收紧** `[P] 可并行`
...
- [ ] **Step 6：G2 gitleaks CI 门禁** `[P] 可并行`
...
- [ ] **Step 7：C1 SQL 注入 grep 门禁** `[P] 可并行`
...
- [ ] **Step 8：L1 积分扣减并发安全核验 + CHECK 约束** `[P] 可并行（billing 域串行起点）`
...
- [ ] **Step 11：L4 计费对账定时任务** `[依赖: Step 8, Step 9]`
