...
- [x] **Step 4：前端与调试可见性**（commit 385ecd55，前端 1096/1096+vue-tsc 0）
  - **目标**：全局模式可辨识、可调试
  - **动作**：①检索调试面板：GLOBAL 分支标识+批数+参与文档数显示；②问答界面全局回答的文档级引用渲染（[1]《标题》无段落锚点，可点跳文档）；③多库提示文案
  - **文件**：`RetrievalDebugPanel.vue`、chat 引用渲染组件、Test ×1
  - **依赖**：Step 2｜**验证**：vitest 渲染分支；手测「总结这个库」全流程 ✅（vitest ×2；手测留 Phase4）
  - **实现注（偏离）**：①「可点跳文档」落为跳知识库模块首页——前端无 doc→kb 深链基建（KnowledgeView 不支持 query 选库/选文档），按 documentId 精确定位需新建查询链路超出本步可见性范围，落 `/knowledge` 由用户选库（实现注记，后续可深化）；②多库提示文案零前端改动——后端 Step2 已在答案尾拼 `>` 提示行，chat/调试面板原样渲染即达意；③调试面板 GLOBAL 分支下 L0/L1/BM25/证据 L2 区自然隐藏（后端返空数组），无需逐区加 v-if
...
- [x] 单测新增 ~8 ✅（实际远超：Step2 后端 13+Step3 后端 5+Step4 前端 2）
- [ ] 手测剧本：「总结整个库」三段结构+文档级引用；50 文档库延迟 <30s；普通问题零影响（黄金集回归）——留 Phase4 统一执行
