...
- Flyway V94 已在真实 PostgreSQL 执行，`asset_versions(file_id)` 部分索引存在。
...
- 修复：新增资产文件共享 grantor，按资产项目 ACL 放行 OWNER/成员、OPEN 和 APPROVED 用户；无可读引用仍拒绝。V94 增加 `asset_versions(file_id)` 部分索引。
- 浏览器复测：`newuser` 打开官方公共项目，5 张图片均加载为有效 blob（naturalWidth 5504/5504/4096/1280/1280）；打开复制目标项目，副本图片 naturalWidth=5504；页面无 403 提示。**已解决**。
...
- 修复：历史 `provider_id=null` 规则按 model 排除所有供应商同名候选；创建 API 使用同一冲突口径。
- 浏览器复测：“新增价表”下拉只显示 2 个尚未配置的 Seedream 图片模型，已有 7 个全局价模型均未出现。**已解决**。
...
- 修复：OWNER 列表最小联查 `users.username`，VO/前端展示用户名并保留 ID 兜底。
- 浏览器复测：审批弹窗显示 `newuser / 已撤销`，不再显示“用户 #4”。**已解决**。

## 结论

- 公众池发布、申请/批准/撤销、公共 VIEWER、成员授权和复制事务主链路已跑通。
- 三项缺陷均已完成 TDD 修复、相关自动化回归、编译/构建和真实浏览器复测；本轮阻断已解除。
- 自动化证据：后端定向回归 40/40，前端 41/41；`mvn compile`、`npm run build` 成功。
