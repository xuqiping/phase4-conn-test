...
> 版本：v1.1（v1.0 初版；v1.1 对齐 Phase 2/3 的「功能联动点清单」+「运维考量清单」——变更前查既有联动与运维埋点不被破坏，回归覆盖联动用例，删功能同步清理运维侧）
...
3. **更新 spec / PRD**：规格是唯一真相源——改 / 删功能后，对应 spec 必须同步改，并**记录"为什么改"**。
4. **追加变更记录**：在 `workflow_output/docs/changes/变更记录.md` 追加一条（日期 / 变更内容 / 原因 / 影响范围 / 涉及文件 / 回滚 commit）。
...
8. **考虑数据迁移与兼容**：删字段 / 改类型，老数据怎么办？是否需要兼容期？写进迁移脚本注释。
   > 📌 表结构说明见各功能 Feature Map 的「数据库表与 SQL 注解（Flyway）」。

### E. 删公开接口 / 功能：废弃流程（先废再删）

9. **不要一刀切删除对外暴露的接口 / 功能**：先标记 **deprecated（废弃）**，保留一段时间 + 给替代方案，再在下一次变更真正删除——给依赖方（别的模块 / 前端 / 第三方）过渡期。

### F. 回归测试（改 / 删必做，区别于新功能）

10. **跑全量自动化测试**（单测 / 集成 / E2E），全绿才继续。
11. **人工回归受影响功能**：照影响评估表里列的受影响项，逐个验证没被改坏。**删功能要把对应的测试也删 / 改**。
    - **【v1.1】受影响功能若有「功能联动点清单」，人工回归必须覆盖其中的联动用例**（含正向 / 反向 / 半选 / 批量），不能只验主流程——改 / 删最容易坏的就是边界联动。

### G. 文档同步（关键——现有流程最易漏的）

12. **改 / 增 / 删后，同步更新所有受影响文档**：
...
13. **验证**（复用 Phase 4）：跑起来，亲眼看改动生效 + 没破坏老的。
14. **commit**：每个变更一个清晰 commit（消息含变更原因），当回滚锚点。部署走 Phase 5。

## 产物

- `workflow_output/docs/changes/影响评估-<变更名>.md` —— 本次变更的影响评估表
- `workflow_output/docs/changes/变更记录.md` —— 追加一条变更记录（项目级 changelog）
