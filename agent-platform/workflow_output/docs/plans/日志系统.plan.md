...
## 安全检查清单
- [x] **PII 红线**：任何 `log.info` 不打印用户输入原文/记忆原文/LLM 响应原文，只打 length/hash/id（#6 防回潮，进评审 checklist）。（RequestLogFilter 不读 body；审计 detail 走 LogMasker + 200 字符截断）
- [x] **脱敏覆盖**：手机号/银行卡/身份证/邮箱/apiKey 五类正则生效，含 JSON 文件输出。（MaskingAppender 事件层打码，console/JSON/SQL 全通道；LogMaskerTest 6 例绿）
- [x] **detail_json 不含 PII 原文**：审计前后值摘要只记字段名+脱敏值。（AuditLogAspect 参数 map 过 LogMasker；AuthService 只记 reason 码）
- [x] **audit_logs 权限**：DB 层 REVOKE UPDATE/DELETE（只允许 INSERT/SELECT）；查询接口 `@RequirePermission("system:audit:read")`。（V78 REVOKE；AuditLogControllerTest 无权限 403 绿）
- [x] **日志中心页**：仅 `system:audit:read` 可见，路由守卫同步加。（菜单隐藏 + 页内 canView + API 403 三重兜底）
- [x] **错误信息**：审计/日志相关异常不外泄内部细节。（审计落库异常仅 WARN + droppedCount，不外抛）
- [x] **新增依赖安全**：micrometer-tracing、logstash-encoder 版本无已知漏洞。（micrometer BOM 托管；logstash-logback-encoder 钉 7.4）
...
- [x] **Step 2：logback 双格式 + SQL 独立通道**（✅ 2026-08-09，commit d017051）
...
- [x] **Step 3：异步上下文传递**（✅ 2026-08-09，commit 80e3b9e）
...
- [x] **Step 4：MdcUserFilter 用户上下文**（✅ 2026-08-09，commit e79c45c）
...
- [x] **Step 5：RequestLogFilter 请求耗时摘要**（✅ 2026-08-09，commit ef997d9）
...
- [x] **Step 6：脱敏 MessageConverter（%mask）**（✅ 2026-08-09，commit e7846b8；实现偏离：改用 MaskingAppender 事件层打码，console/JSON/SQL 全通道一次生效，见备注②）
...
- [x] **Step 7：sidecar structlog 对齐** `[P]`（✅ 2026-08-09，commit 24dd9dd）
...
- [x] **Step 8：audit_logs 建表 + 实体/Mapper**（✅ 2026-08-09，commit 093ce53；V77 全字段建表，超集覆盖安全体系 S1 最小表，见备注③）
  - **目标**：审计存储就绪（LOG-FR-09）。
...
- [x] **Step 9：@AuditLog 注解 + AOP 异步落库**（✅ 2026-08-09，commit 549a38b）
...
- [x] **Step 12：审计保留策略与 DB 权限收紧**（✅ 2026-08-09，commit bf06c26；V78 DO 块判 agent_app 角色存在再 REVOKE，本地超管环境跳过不炸）
...
- 偏离计划处在此注明原因。
- ① **Step 4 偏离**：MdcUserFilter finally 不做 `MDC.clear()`，改为只 remove 自身 3 个 key（userId/username/clientIp）——防抹掉 micrometer tracing 的 traceId 作用域导致断链；防串号由「只清自身 key + 每次请求覆盖写」保证。
- ② **Step 6 偏离**：未用 `%mask` ClassicConverter，改用 MaskingAppender（AppenderAttachable 事件层装饰，同 AsyncAppender 模式）——一条代码路径同时覆盖 console 文本、LogstashEncoder JSON、SQL 通道；converter 方案对 JSON encoder 不生效需另写 JsonDecorator，双机制易回潮。
- ③ **Step 8 冲突解决**：audit_logs 占用 V77 且为全字段超集（含 username/client_ip/user_agent/result），安全体系 S1 计划的「最小 audit_logs 表」作废——S1 迁移须改号 V80+ 并直接复用本表（已同步进 V77 注释；S1 plan 待改）。
- ④ **Step 3 偏离**：Spring ThreadPoolTaskExecutor 只收一个 TaskDecorator，新增 CompositeTaskDecorator 组合 MdcContextTaskDecorator + BillingContextTaskDecorator；裸 ExecutorService（UsageCollector）走 `MdcContextTaskDecorator.wrap()` 静态包装。
- ⑤ **SQL 通道免登记**：mybatis-plus `log-prefix: sql.` 使全部 mapper 包（现状+未来）日志归 `sql` logger，logback 单 logger 覆盖，无需逐包登记。
