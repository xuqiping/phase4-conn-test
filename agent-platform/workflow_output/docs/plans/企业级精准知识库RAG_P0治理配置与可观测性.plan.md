...
- [x] **Step 1：建立 Pipeline、检索运行和模型调用主数据**
...
- [x] **Step 2：实现可插拔 Ranking 配置和显式模型解析**
  ...
  - **文件（≤20）**：
    - `backend/src/main/resources/db/migration/V103__rag_ranking_config.sql`
...
- [x] **Step 3：统一 RAG Trace 上下文与 MDC 传播**
  - **对应需求**：RAG-FR-08
