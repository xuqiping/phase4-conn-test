...
- [x] **Step 3：ConnectorSyncWorker**（代码+测试全落，后端全量 2909/2909）
  - **目标**：定时增量同步入既有解析索引管线
  ...
  - **依赖**：Step 2｜**验证**：单测——增量三态 ✅/并发认领 ✅/错误上限 ✅/限速（Step2 FetchLimiter 已测）✅；集成 mock 站点两轮同步——编排层单测覆盖（首轮全新增/次轮 1 改 1 增 1 删），真实站点全链路留 Phase4 手测
  - **实现注（偏离）**：①**ISOLATED 落地=复用 QUARANTINED+原因前缀分流**（spec「走既有隔离治理」）：`isolateDoc` 置 QUARANTINED+`连接器源端已删除（ISOLATED）` 前缀+**清节点与三池向量**（召回以节点为准=不召回；管理端可解除）；源恢复且 etag 未变→`reviveDoc` 复活重解析（unquarantine 同构）；**安全隔离（注入扫描命中）按前缀识别绝不触碰**（含自动复活）；②**变更=既有版本链**：createVersion（版本+1 旧版可回溯）→activate→`resetDocForResync` 换 fileRef/fileHash+置 PENDING+**清旧节点与向量**（writeNodes 不清旧——quarantine→unquarantine 重解析同构坑）+事务内发 DocumentUploadedEvent 走完整解析管线；③**新增走 upload() 全管线**：store 白名单/magic 嗅探/配额/初始版本/解析事件一处不漏——html/htm 上传白名单 14x-4 已含，连接器白名单 ⊆ 上传白名单；byte[] 经 `InMemoryMultipartFile` 适配而非旁路；**KnowledgeDocumentService 零改动**（比计划「最小适配」更净）；④认领=**乐观口径短事务**：行锁（SKIP LOCKED）窗口只盖「复核 ENABLED+last_sync_at 仍是读到的旧值→推进 now」（同步分钟级不占行锁，比 IndexJobWorker 长锁模式适配本表无 locked_until 列的现状）；⑤**V176 补 sync_error_streak 列**（计划未列：连续错误计数须落库，跨轮存活）；错误语义：轮级异常（连不上/枚举败）与「全败零进展轮」计 streak，部分成功轮=成功收口但摘要带错误数（防坏凭证空转刷 ERROR）；⑥自愈扩展（计划外）：etag 未变+status=FAILED→同文件重解析重试；etag 未变+ISOLATED→复活——两者零网络成本；⑦单轮预算 **50 动作**（新增/变更/复活/重试全计——都触发解析计费），externalId 排序确定性消化超量；cron 抖动=确定性偏移 [0,600)s（id 散列）；⑧摘要/日志脱敏 `sanitize`：剥 URL userinfo（user:pass@→***@）+压空白+截 400——凭证与内网信息不落 last_sync_summary；⑨S3/WebDAV/URL 生产 SSRF 装配落 `ConnectorFactory`（S3 endpoint 建客户端前 validate——SDK 路径不走 SafeHttpFetch）；⑩坑：Spring CronExpression API 是 `next()` 非 `nextExecution()`；Mockito `never()` 在 Mockito 类不在 ArgumentMatchers；MP 纯单测 Lambda 列名解析须 `TableInfoHelper.initTableInfo`（AssetServiceTest 同款）+wrapper 断言须先 `getSqlSegment()` 再读 paramNameValuePairs；⑪测试 +15（计划 ×2）：worker 编排 10（到期认领门/认领被拒不跑/首轮新增过 upload 管线+文件名与 contentType/增量三态 变·增·源删 ISOLATED/开关开走治理删除链/手工删+用户删不复活/安全隔离不动且计轮败/复活+FAILED 自愈/50 动作闸/连不上记账脱敏）+tx 记账 5（第 3 轮置 ERROR·第 2 轮不置/成功清零/并发推进弃权/行缺失弃权）。

- [x] **Step 4：前端 ConnectorPanel**
  - **目标**：连接器可视管理
