...
- [x] **Step 13：节点 @引用机制**（vue-tsc 绿 + vitest 194/194 零回归，+34 新测）
  - **对应需求**：FR-012
  ...
  - **落地**：`utils/interpolate.ts`（`ancestors` 反向 BFS+visited 防环 / `parseMentions` 解析 `@{{node\|asset:id}}` / `interpolate` 不递归替换+断链降级「【断链】」/ `findBrokenMentions` 断链检测 / `uniqueLabel` L9 查重序号，19 单测）；`MentionTextarea.vue`（原生 textarea+selectionStart 精准光标，`@` 唤起→候选 label 过滤→键盘上下/Enter/Esc→插入占位符+尾随空格，防 foo@bar 误判，10 单测）；`PropertyPanel.vue`（text/script/video 三处 prompt/synopsis 换 MentionTextarea；新增 `candidates/brokenMentions/allLabels` 三 prop；断链黄色 warn；名称框 @blur 走 `uniqueLabel` 重命名查重 L9，+5 测共 12）；`CanvasView.vue`（`selectedAncestors`/`mentionCandidates`/`otherLabels`/`brokenMentions` 四 computed 读 boardRef.getEdges().value 响应式追踪；`buildMentionResolver` 按节点类型注入 outputText/scenes 序列化/prompt+元信息；`interpolateForRun` 拷贝 data 不污染原文；onRunNode/onRunVideo 运行前插值+断链 warning L7/L8）；`CanvasBoard.vue`（addNode label 去重新建/粘贴入口 L9）；`types/canvas.ts` 加 `MentionCandidate`。**占位符本体存 node id（重命名不断链 L8）；不递归（防 A@B、B 含 @ 死循环）；环画布祖先集构造不出→选择器空+提示解环。MVP 选择器仅列祖先节点，asset 占位符引擎支持但运行期不预解析（选择器不产 asset 占位符，Phase4 评估）**

- [x] **Step 14：整体验证 + 文档收尾**（vue-tsc 绿 + vitest 194/194；4 件套产出）
  - **对应需求**：全部 FR
  ...
  - **依赖/并行**：依赖全部；B7 串行
  - **验证**：下方「整体验证」清单全勾
  - **落地**：4 件套全产出——[速查表25](../../../项目工程文档/项目功能介绍/速查表/25-项目资产库.md)（技术速查）+ [FeatureMap](../feature-map/项目资产库.feature-map.md)（代码-功能地图+大白话注解+踩坑批注）+ [UserOps](../user-ops/项目资产库用户操作手册.md)（B类，功能→步骤→界面变化→预期+权限矩阵+异常恢复）+ [README](开发进度/项目资产库/README.md)（B/C类，谁用/场景/效益+核心概念+技术说明）。后端 asset 包 72/72 单测绿复核；前端 vue-tsc 绿 + vitest 194/194 零回归。**整体验证清单 Phase4 项**（关键路径 E2E / L1-L10 手测 / 真库 V56-V59 执行 / FR 对齐）留 Phase4。

## 整体验证（功能级）

- [x] 所有单测通过（用例带 FR 编号）；asset 包测试绿（后端 75/75 + 前端 38/38 + vue-tsc 0，Phase4）
- [x] 关键路径 E2E：建项目→授权→上传/入库→双轴筛选→定稿→画布引用→@引用→拼接成片（API 38 绿 + L5/L6 真链 + runtime 9/9，拼接成片属无限画布 C13 不在本功能）
- [x] 安全检查清单全部完成并验证（重点：viewer 越权写、fileId 遍历、无 asset:write 直访 API）—— 6 越权向量全 403，双层授权平台+数据层兜底验证
- [~] 功能联动点 L1~L10 逐条手测（含反向/边界）—— API 正向 + 边界已验（L1 转让/L2 LOCK/L3 归档隐藏/L4 级联软删/L5 入库三态/L6 resolve 版本隔离+REFERENCE/L10 五桶/@引用空祖先 graceful），**全反向边界 + 各上传真文件留真人手测并行进 P5**
- [x] 与设计方案对齐复核（每个 FR 有 Step、每条联动有验证）—— 第二 AI 审查对照 FeatureMap/UserOps/plan，修 1🔴+2🟠+3🟡，6🟡 记档
