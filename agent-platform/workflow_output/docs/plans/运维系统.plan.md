...
- [x] **Step 3：LLM 指标埋点**（对应需求：OPS-FR-03）
...
- [x] **Step 4：工作流 + 登录指标埋点**（对应需求：OPS-FR-04、OPS-FR-07）
  - **目标**：工作流终态与登录结果可计量。
...
- [x] **Step 5：索引队列 + 记忆管线指标**（对应需求：OPS-FR-05、OPS-FR-06）
  - **偏离备注**：plan 原文落点 `chat/service/MemoryService.java` 系计划12 重设计前旧架构；实际埋点为 `MemoryConsolidationWorker.processScope`（总结管线）+ `MemoryAssetIngestWorker.poll`（文件摄入管线），指标语义不变。
  - **目标**：5s 轮询黑盒与记忆管线故障可见。
...
- [x] **Step 6：sidecar 指标**（对应需求：OPS-FR-08）
  - **偏离备注①**：instrumentator 锁 **6.0.0**——7.x 强拉 starlette≥0.38 与 fastapi 0.111.0（starlette<0.38）硬冲突；6.0.0 兼容 starlette 0.37.2。
  - **偏离备注②**：本机 sidecar 运行/测试解释器为 `C:\Python314\python.exe`（依赖全齐）；`D:\IT\python\envirenments\python.exe` 是 3.8 残环境（装不了 langgraph 1.2.2，弃用）。
  - **偏离备注③**：`tests/test_callback_client.py` 3 条失败系**存量环境性失败**（干净 HEAD 复跑同样 3 红——真发 HTTP 到不可解析域名），与本 chunk 无关，未修。
  - **排雷沉淀**：sse-starlette 2.x `AppStatus.should_exit_event` 全局 anyio.Event 只建一次绑首个 loop，套件出现第二个流式测试必炸 "bound to a different event loop"——test_metrics.py 以 autouse fixture 用后重置 None。
...
- [x] **Step 7：健康检查聚合**（对应需求：OPS-FR-09）
  - **偏离备注①**：DiskSpaceHealthIndicator 采**自定义 bean 顶掉 Boot 默认版**（已反编译确认 Boot 3.2.5 `DiskSpaceHealthContributorAutoConfiguration` 为 `@ConditionalOnMissingBean(name="diskSpaceHealthIndicator")` 让步），而非 yml 调阈值——yml `management.health.diskspace.threshold` 是绝对字节数，表达不了「可用 <15%」百分比语义。
  - **偏离备注②**：`RagRetrievalServiceTest`、`RuntimeCallbackSecurityTest` 各 1 红系**存量失败**（stash 后干净 HEAD 复跑同红），与本 chunk 无关，未修。
  - **环境备注**：本机 dev 库 flyway_schema_history 系改号前旧分支写入（V65/67/68=memory 系 vs 当前 V65/66=billing、V71+=memory），validate 必败；已跑 `flyway:repair` 对齐 checksum，但「applied-not-resolved 68 / resolved-not-applied 66」须用户决策（重建 dev 库或手工改号补登），未擅动。集成测试上下文装配验证改用 ApplicationContextRunner 无 DB 方案（HealthIndicatorWiringTest）。
...
- [x] **Step 8：监控组件安装与服务化**（对应需求：OPS-FR-10）
  - **偏离备注（2026-08-10 实装）**：版本定为 prometheus 3.13.2 / alertmanager 0.33.1 / grafana 12.3.1 OSS / windows_exporter 0.31.8 / WinSW v2.12.0（NET461）；安装根统一 `D:\IT\ops\{组件}\`，四组件全部只监听 127.0.0.1（与 /actuator 同安全策略）。踩坑三则：①windows_exporter 0.31 已移除 `cs` 采集器（unknown collector 起不来，xml 已删并留红线注释）；②WinSW v2 `status` 对未安装服务也返回 0，判存必须用 `sc query`（已写进 install-services.bat 头注释）；③alertmanager 占位配置 receiver 必须写字符串 `"null"`，YAML 裸 `null` 解析为空值报 "root route must specify a default receiver"。另：windows_exporter 直接用官网 exe（非 msi），WinSW 资产名是 `WinSW.NET461.exe`（无 .x64 后缀）。
  - **验证留痕**：install-services.bat 一键注册 4 服务（agent-prometheus/alertmanager/grafana/windows-exporter，Automatic+崩溃 10s 重启）；Prometheus targets 4/4 UP（backend/sidecar/windows_exporter/prometheus）；自定义指标抽查 auth_login_total、knowledge_index_queue_depth、jvm_*、windows_* 均入仓。sidecar target 依赖 sidecar 进程在跑（日常 down 属预期，非故障）。
...
- [x] **Step 9：四个预置看板**（对应需求：OPS-FR-11）
  - **偏离备注（2026-08-10 实装）**：①看板全部中文（用户要求：标题/图例/说明全中文，folder=运维监控，uid 固定 ops-system-overview/ops-api-quality/ops-ai-core/ops-security）；②JSON 不手写——生成器 `gen_dashboards.py` 入库（改面板改脚本重跑，避免手维护大 JSON 漂移）；③数据源 provisioning uid 固定 `prometheus`，看板按 uid 引用；④micrometer 计数器懒注册——llm_calls_total 等指标首次调用后才出现，对应面板空属预期非故障（看板描述已注明）；⑤prometheus.yml 的 uri 面板排除 /actuator.* 防自抓取噪音。
  - **验证留痕**：Grafana 重启后 provisioning 自动加载四看板入「运维监控」文件夹（API /api/search 实证）；数据抽查：http_server_requests 6 序列（含 /api/auth/login、register 真实流量）、windows_cpu 80、jvm 6、auth_login_total 2、security_endpoints_unguarded=0；sidecar 图执行/LLM 面板待真实调用出数。
...
- [x] **Step 10：告警规则 + 通知通道**（对应需求：OPS-FR-12）
  - **偏离备注（2026-08-10 实装）**：①规则落盘 8 条非 7 条——"up==0 1min critical"拆成后端 critical(1min)/sidecar warning(5min)（sidecar 服务化前日常不跑属预期，一刀切 critical 会告警疲劳）；②"单 IP 登录失败>10/min"改全局失败速率近似（IP 是高基数红线严禁进 metric tag，单 IP 判定归安全体系限流/审计）；③钉钉机器人拒收 Alertmanager 原生报文，须加转译适配器 prometheus-webhook-dingtalk v2.1.0（第 5 个 WinSW 服务 agent-dingtalk-webhook，127.0.0.1:8060）；④webhook 不入库不入 yml 手填——中文引导脚本 `配置钉钉告警.bat` 双击填地址自动写配置+重启服务+发测试消息（用户要求"界面上留设置"，Alertmanager 无 UI，此为等价入口）；⑤Grafana 挂 Alertmanager 数据源（uid alertmanager）可在中文界面看告警状态/挂静默；⑥LLM 失败率规则加最小流量门槛防低流量抖动。
  - **验证留痕**：promtool check config/rules 全过；重启后 8 规则加载；kill 后端实机验收——规则 firing → Alertmanager active → 适配器转发钉钉 API 报 "token is not exist"（占位 token 预期失败），**链路 Prometheus→Alertmanager→适配器→钉钉 API 全通，只差用户跑 bat 填真 webhook**；恢复后 resolved。期间发现 spring-boot:run 的 maven 父进程会复活 fork 的子 JVM，kill 测试须杀进程树（排雷：查真正监听进程用 Get-NetTCPConnection.OwningProcess）。
...
- [x] **Step 11：备份脚本 + 计划任务 + 异地副本 + 演练规程**（对应需求：OPS-FR-13~17）
  - **偏离备注（编码红线实证）**：含中文注释的 .bat 在 zh-CN Windows 上**必须 GBK 编码 + CRLF + 无 BOM + 不 chcp 65001**——UTF-8+chcp 65001 时 cmd 批解析器多字节错位致 REM 行漏出执行（实测 'ows'/'last-backup-status.txt' 等碎片被当命令）；加 BOM 则 `@echo off` 直接炸（'﻿@echo' 不识别）；LF 行尾 REM 保护整体失效。告警 webhook JSON 内容一律 ASCII（GBK 字节发钉钉会乱码）。
  - **验证留痕**：本机真跑——backup-pg.bat 产出 dump 283KB + 周日归档周备 + last-backup-status.txt=OK；pg_restore 进 agent_platform_drill 抽查 users=1 后清理；backup-files.bat 探针文件增/删两轮验证 /MIR 镜像语义正确；sync-offsite.bat 未配 OFFSITE_TARGET 走失败兜底（状态文件 FAIL+exit 1）。
...
- [x] **Step 12：deploy.bat + Flyway 门禁 + 回滚策略**（对应需求：OPS-FR-18~20）
  - **偏离备注（2026-08-10 实装）**：①服务器无 Maven，Flyway 门禁实现为「后端启动自动迁移 → 迁移失败=启动失败=健康轮询超时=自动回滚」，`flyway:repair` 纪律改为"发布前须在开发机对齐 checksum"写入脚本头注释；②纯前端发布走快道（robocopy dist + nginx -s reload，不动后端服务）；③用法 `deploy.bat 新jar [新dist]` / `deploy.bat NONE dist`；④失败告警直接 POST 钉钉适配器（未配置时静默失败）；⑤发布布局按 99-部署要求：C:\agent-platform\{backend,frontend\dist,versions\时间戳}。
  - **踩坑沉淀（重要）**：**git-bash 的 sed 处理 GBK+CRLF bat 会丢 \r**，导致 REM 注释保护失效、中文碎片被当命令执行——改 GBK bat 一律用 PowerShell `[IO.File]::ReadAllBytes/WriteAllBytes` + GetEncoding('GBK') 字节级读写。
  - **验证留痕**：参数检查/用法分支干跑正确；模拟布局（C:\Windows\Temp\ap-test 改写 ROOT）实测纯前端路径：旧版备份进 versions\时间戳（含旧 jar+旧 dist）、robocopy /MIR 新包替换正确（新增有/删除无）、版本目录滚动保留——全对。后端停启/健康轮询/回滚分支与首跑真实发布待服务器窗口（C:\agent-platform 布局与 agent-backend 服务尚未建）。
...
- [x] **Step 13：《运维手册》+ WinSW 兜底**（对应需求：OPS-FR-21、OPS-FR-22）
  - **偏离备注（2026-08-10 实装）**：①VictoriaLogs 未引入本栈（日志走 PG audit_logs + 文件日志），无对应 xml，仅 backend/sidecar 两个模板；②sidecar xml 绑 127.0.0.1（99-部署要求原文 0.0.0.0，与 /actuator 同安全策略收紧）；③**本机不注册 agent-backend/agent-sidecar 服务**——本机 dev 后端走 start-backend.ps1 占 8080，注册即端口冲突；模板入库，注册动作留真实服务器窗口（与 99-部署要求「开发机构建/服务器运行」模型一致）；④backend.xml 的 env 密钥段全占位符，真实值只写服务器本地。
  - **验证留痕**：手册含日/周/季巡检 + 8 告警处置速查 + 4 runbook（后端宕机/LLM 失败/队列积压/线程打满）+ 密钥泄露应急五步走（衔接安全体系轮换）；服务清单 7 项（5 监控组件已 RUNNING + 2 模板待服务器注册）。kill 服务进程自动拉起属 WinSW 既有行为（监控五组件 RUNNING 即实证），agent-backend 的等价验证并入真实服务器窗口首跑。
