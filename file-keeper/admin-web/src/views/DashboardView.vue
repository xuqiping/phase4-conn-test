    ...
    <n-spin :show="loading">
      <n-grid :cols="1" :x-gap="16" :y-gap="16" responsive="screen" item-responsive>
        ...
        <n-gi span="1" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px">
          ...
          <n-card size="small" hoverable class="kpi-card">
            <n-statistic label="正常用户" :value="stats?.activeUsers ?? 0" />
          </n-card>

          <n-card size="small" hoverable class="kpi-card">
            <n-statistic label="已禁用用户" :value="stats?.disabledUsers ?? 0" />
          </n-card>
...
import {
  NCard, NGrid, NGi, NStatistic, NH2, NSpace, NButton, NSpin, NProgress, useMessage
} from 'naive-ui'
