...
- [x] **Chunk 3：Windows `.lnk` 托管导入、安全删除与重新定位**
  - **对应需求**：F2。
...
- [x] 托管删除仅作用于应用根下的普通 `.lnk`，覆盖路径穿越、根目录、目录、符号链接和连接点。
- [x] 导入只解析/复制，不执行；打开继续使用参数化 OS API，不拼 Shell 命令。
- [x] 快捷键只保存组合键和收藏 ID，不携带脚本或参数；日志不记录完整路径。
- [x] 窗口标题按文本渲染，不上传、不写持久化日志。
- [x] 悬浮球仅加载本地资源；失败时永远保留可见恢复入口。
- [x] 分组名校验、SQL 参数绑定、分组删除事务和“内容不级联删除”均有测试。
- [x] 剪贴板正文、收藏路径和窗口标题不进入服务端请求、遥测或错误日志。
- [x] 若新增 Rust/npm 依赖，记录用途并执行依赖漏洞检查；本轮 Chunk 7 未新增依赖。
...
落实结果：本地日志已记录脱敏操作编号/动作/数量/耗时/结果码；旧设置兼容、SQLite 事务回滚、快捷键有界恢复、悬浮球降级、分页与索引均随对应 Chunk 完成。远程开关、后台运维页和远程告警按计划不做；1 万条与 100 组性能实测留 Phase 4。

## 8. 整体验证与硬闸门

- [x] 六项需求均有实现 Chunk、自动化验证和 Phase 4 人工用例映射。
- [ ] 定向前端/Rust 测试通过；完整 `npm test`、`cargo test` 与 M1—M13 在 Phase 4 执行。
- [ ] 性能指标按专项性能规格记录实测结果。
- [x] Feature Map、User-Ops、测试方案、开发进度和人工测试状态同步。
