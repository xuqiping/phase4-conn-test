...
| ID | 级别 | 缺陷 | 状态 |
|----|------|------|------|
| P0-1 | 根本 | 对称签名密钥必然驻留客户端 → 可伪造任意 token | 🚧 待修（plan Chunk 1） |
| P0-2 | 根本 | JWT secret 默认兜底硬编码 / 服务端默认空串 | 🚧 待修 |
| P1-1 | 高 | token 绝对过期时间被解析但未校验 → 可无限续期 | 🚧 待修（plan Chunk 1） |
| P1-2 | 高 | 特权命令（杀进程/截图/剪贴板）Rust 侧无二次校验 | 🚧 待修（plan Chunk 2） |
| P2-1 | 中 | 设备身份全客户端生成 → 可刷试用 / 绕设备上限 | 📋 待排期 |
| P2-2 | 中 | 过期依赖 JS `Date.now()`，时钟可回拨 | 📋 待排期 |
