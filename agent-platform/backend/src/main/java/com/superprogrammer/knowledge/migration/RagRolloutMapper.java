...
public interface RagRolloutMapper {
    @Insert("""
            INSERT INTO rag_rollout_states(kb_id,current_percentage,current_config_version,current_operator_id,
              previous_percentage,previous_config_version,previous_operator_id,updated_at)
            VALUES(#{kbId},#{currentPercentage},#{currentConfigVersion},#{currentOperatorId},
              #{previousPercentage},#{previousConfigVersion},#{previousOperatorId},NOW())
            ON CONFLICT (kb_id) DO UPDATE SET current_percentage=EXCLUDED.current_percentage,
              ...
              previous_operator_id=EXCLUDED.previous_operator_id,updated_at=NOW()
            """)
    ...
    class RolloutRow {
        ...
        public Long currentOperatorId; public Integer previousPercentage; public String previousConfigVersion;
        public Long previousOperatorId;
        public static RolloutRow of(RagRolloutService.RolloutState current, RagRolloutService.RolloutState previous) {
            ...
            if(previous!=null){row.previousPercentage=previous.percentage();row.previousConfigVersion=previous.configVersion();row.previousOperatorId=previous.operatorId();}
            return row;
        }
    }
}
