...
- [x] **Step 7：IPC 层与事件订阅 + 项目创建向导** ✅ 2026-08-14（5 commands + kernel://state 事件 + 向导 + PhaseBar，前端 9/9 测试过含 AC-032 UI 层与 AC-052 恢复）
...
- [x] **Step 8：驾驶舱静态版** ✅ 2026-08-14（HUD 四指标 + 轮次时间线 + 1000 条虚拟列表，3 测试过；进度读状态机真实数据）
  - **对应需求**：FR-039（基础）
...
- [x] **Step 9：双端打包 + 自动更新 + CI** ⚠️ 2026-08-14 部分完成：本地 Win MSI 出包验证通过（3.8MB）+ 双端 CI workflow 就位；**Updater 签名密钥 / Apple 证书 / Win 证书待用户提供**（secrets 就位后 CI 自动签名公证）
  - **对应需求**：FR-018（交付部分）
