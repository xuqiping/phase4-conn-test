...
param(
    # 部署约定根（与部署手册终 §四 一致）
    [string]$AppRoot = "E:\agent-platform",
    [string]$NginxConf = "C:\nginx\conf\nginx.conf",
    [string]$PgDataDir = "C:\Program Files\PostgreSQL\16\data",
    # Redis/Memurai 配置（找到哪个算哪个）
    [string[]]$RedisConfCandidates = @("E:\IT\ops\memurai\memurai.conf", "C:\Program Files\Memurai\memurai.conf", "E:\IT\ops\redis\redis.windows.conf"),
    ...
    [string]$FimBaselineFile = "E:\agent-platform\ops\fim-baseline.csv",
    [switch]$Rebaseline
)
