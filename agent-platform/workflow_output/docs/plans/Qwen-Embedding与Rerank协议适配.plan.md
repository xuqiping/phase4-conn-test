...
- [ ] API Key 仅从 AES 解密后的运行时对象使用，不写日志/响应/测试快照。
- [ ] 上游错误只返回安全摘要；完整响应正文不进入前端、Trace 或业务日志。
- [ ] Query、documents、Prompt、Chunk 正文不写日志和 MDC。
- [ ] 响应索引、数量、维度严格校验，异常 fail-closed。
- [ ] RERANK 不进入 CHAT/EMBEDDING 路由，不允许隐式模型替换。

## 运维考虑清单

- [ ] Embedding/Rerank 日志带 traceId、modelRequestId；Ranking 路径带 retrievalRunId/rankingRunId。
- [ ] configuredMode 与 effectiveMode 分开记录；fallback 仅按既有配置执行。
- [ ] 指标记录 provider/model/result/latency，不使用 query、用户 ID 等高基数标签。
- [ ] 计费失败继续吞异常，不回滚已成功的模型响应。
- [ ] 管理页真实测试可作为部署后健康检查，不需要导出密钥。
