...
- [x] 单测新增 ~20（关系 8/注入 8/建议 2/前端 2）→ 实际 **74**：关系 10（Step1）+processor 12+merge 4（Step2）+建议 18（Step3）+C1 前端 10（Step4）+附件入库 13（Step5）+注入 11（Step6）+C2 前端 6（Step7）；全量后端 2803/2803 + 前端 1093/1093 + vue-tsc 0
- [x] 集成：无边无附件时管道输出与基线逐字节一致（WP1 自身的零回归门）——Step2 merge 4 用例（无边时 planExpansion 空 → 输出同基线）+ 既有全量零回归；Step6 走 step8 装载同一通道，非附件证据 content 不变（service 测试 2 断言）
- [ ] 手测剧本：术语表 MUST_CITE 差旅制度→问差旅带术语表引用；架构图附件→问图内容；建边后旧缓存答案失效重算；txt/pdf/图片三型 nodes=1 —— **留 Phase4 统一跑**（需真文件入库+真 VLM，对应测试方案 L2/L4/L6 用例）
