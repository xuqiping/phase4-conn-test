...
- [x] 黄金集评估门禁不降（C1-C4 各 ≥5 例先建用例再实现）→ 合成黄金集建毕：多轮 maxRounds 1→2 Recall 0.750→1.000/MRR 0.750→1.000（四类语义等价划分）；C4 影子对比真实语料 36 例已跑（A 0.9444→B 0.9722 名义，逐例对账=评测噪声、检索层零变化、无伤害，默认开关维持 true——[开发进度11](../../开发进度/知识库RAG检索能力增强/开发进度11.md)）；**真实库全量跑 Phase4**（评测中心 V115 数据集在库）
