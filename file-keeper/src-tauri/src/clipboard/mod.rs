...
impl ClipboardService {
    pub fn new(database_path: PathBuf) -> Result<Self, String> {
        let cache_dir = database_path.parent().map(|parent| parent.join("clipboard-cache"));
    ...
    pub fn get_item_summary(&self, id: &str) -> Result<Option<ClipboardItemSummary>, String> {
        self.storage.lock().map_err(|err| err.to_string())?.get_item_summary(id)
    }
    ...
    pub fn collect_file_snapshot(&self, paths: &[String], size_overrides: Option<&[i64]>) -> Result<String, String> {
        if paths.is_empty() {
            return Err("没有可记录的文件路径".to_string());
        }
        if paths.len() == 1 && is_image_path(&paths[0]) {
            return self.collect_image_file_snapshot(&paths[0], size_overrides.and_then(|values| values.first().copied()));
        }
        ...
        for (index, path) in paths.iter().enumerate() {
            ...
            let size_bytes = size_overrides
                ...
                .unwrap_or(0);
            let is_directory = metadata.as_ref().map(|value| value.is_dir()).unwrap_or(false);
            let cached = if should_cache_path(path, size_bytes, is_directory, &settings) {
                self.copy_to_cache(path, "files", &settings).ok().flatten()
            } else {
                None
            };
            let copy_state = if cached.is_some() { "cached" } else { "reference_only" };
        ...
        self.storage.lock().map_err(|err| err.to_string())?.insert_file_item(&entries, &title, cached_bytes)
    }

    fn collect_image_file_snapshot(&self, path: &str, size_override: Option<i64>) -> Result<String, String> {
        let settings = self.load_settings()?;
        let metadata = std::fs::metadata(path).ok();
        let size_bytes = size_override.or_else(|| metadata.as_ref().map(|value| value.len() as i64)).unwrap_or(0);
        let is_directory = metadata.as_ref().map(|value| value.is_dir()).unwrap_or(false);
        let cached = if should_cache_path(path, size_bytes, is_directory, &settings) {
        ...
        let image_path = cached.as_ref().map(|(cached_path, _)| cached_path.clone()).unwrap_or_else(|| path.to_string());
        let reader = image::ImageReader::open(&image_path).map_err(|err| err.to_string())?;
        let format = reader.format().map(|format| format.extensions_str()[0].to_string()).unwrap_or_else(|| image_extension(path));
        let image = reader.decode().map_err(|err| err.to_string())?;
        let cache_bytes = cached.as_ref().map(|(_, bytes)| *bytes).unwrap_or(0);
        let id = self.storage.lock().map_err(|err| err.to_string())?.insert_image_item(&image_path, image.width() as i64, image.height() as i64, &format, cache_bytes)?;
        if settings.enable_ocr {
            if let Ok(text) = ocr::recognize_image(&image_path) {
                if !text.trim().is_empty() {
                    let _ = self.update_ocr_text(&id, &text);
    ...
    fn copy_to_cache(&self, source: &str, subdir: &str, settings: &ClipboardSettings) -> Result<Option<(String, i64)>, String> {
        let Some(cache_dir) = self.effective_cache_dir(settings) else {
            return Ok(None);
        };
        ...
        Ok(Some((target_path.to_string_lossy().to_string(), bytes as i64)))
    }

    fn effective_cache_dir(&self, settings: &ClipboardSettings) -> Option<PathBuf> {
        settings.backup_directory.as_ref()
    ...
    pub fn get_file_copy_paths(&self, id: &str) -> Result<Option<Vec<String>>, String> {
        if let Some(files) = self.get_files(id)? {
            let paths = files.into_iter()
                .filter_map(|file| usable_text(file.cached_path).or_else(|| usable_text(Some(file.original_path))))
                .collect::<Vec<_>>();
            if !paths.is_empty() {
    ...
    pub fn update_note(&self, id: &str, note: Option<&str>) -> Result<Option<String>, String> {
        self.storage.lock().map_err(|err| err.to_string())?.update_note(id, note)
    }
    ...
    pub fn get_copy_text(&self, id: &str) -> Result<Option<String>, String> {
        ...
        let (html, markdown, url, _url_title, _url_description, color_hex, color_rgb) = self.get_rich_fields(id)?;
    ...
    pub fn save_settings(&self, settings: &ClipboardSettings) -> Result<ClipboardSettings, String> {
        ...
        normalized.backup_directory = normalize_backup_directory(normalized.backup_directory.as_deref())?;
        self.storage.lock().map_err(|err| err.to_string())?.save_settings(&normalized)?;
        Ok(normalized)
    }
...
fn unique_backup_path(target_dir: &Path, source_path: &Path) -> PathBuf {
    let stem = source_path.file_stem().and_then(|value| value.to_str()).unwrap_or("clipboard-file");
    let extension = source_path.extension().and_then(|value| value.to_str());
    for index in 0.. {
        let suffix = if index == 0 { "备份".to_string() } else { format!("备份{}", index + 1) };
...
fn should_cache_path(path: &str, size_bytes: i64, is_directory: bool, settings: &ClipboardSettings) -> bool {
    matches!(settings.file_save_mode, ClipboardFileSaveMode::Backup)
        && !is_directory
        && cache::extension_allowed(path, &settings.file_extension_mode, &settings.file_extensions)
        && cache::within_item_size_limit(size_bytes, settings.item_size_limit_mb)
}

fn is_image_path(path: &str) -> bool {
    matches!(
        Path::new(path).extension().and_then(|value| value.to_str()).map(str::to_ascii_lowercase).as_deref(),
...
mod tests {
    ...
    fn temp_service() -> (ClipboardService, PathBuf) {
        let root = std::env::temp_dir().join(format!("file-keeper-clipboard-test-{}", uuid::Uuid::new_v4()));
    ...
    fn resolves_copy_text_for_url_color_html_file_and_image_items() {
        let service = ClipboardService::in_memory().unwrap();
        let url = service.add_text_for_testing("https://example.com/path", None).unwrap();
        let color = service.add_text_for_testing("#22c55e", None).unwrap();
        let html = service.add_html_for_testing("<p><strong>Hello</strong><br>World</p>").unwrap();
        let file = service.add_file_for_testing("C:/Users/me/report.pdf", 1024).unwrap();
        let image = service.storage.lock().unwrap().insert_image_item("C:/Users/me/image.png", 120, 80, "png", 4096).unwrap();

        service.update_ocr_text(&image, "image text").unwrap();

        assert_eq!(service.get_copy_text(&url).unwrap().as_deref(), Some("https://example.com/path"));
        assert_eq!(service.get_copy_text(&color).unwrap().as_deref(), Some("#22C55E"));
        assert_eq!(service.get_copy_text(&html).unwrap().as_deref(), Some("**Hello**\nWorld"));
        assert_eq!(service.get_copy_text(&file).unwrap().as_deref(), Some("C:/Users/me/report.pdf"));
        assert_eq!(service.get_copy_text(&image).unwrap().as_deref(), Some("image text"));
    }
    ...
    fn copied_files_are_backed_up_when_cache_dir_exists() {
        ...
        let id = service.collect_file_snapshot(&[source.to_string_lossy().to_string()], None).unwrap();
        ...
        assert_eq!(service.get_item_summary(&id).unwrap().unwrap().cache_state, CacheState::Cached);
    }
    ...
    fn custom_backup_directory_is_used_for_files() {
        ...
        assert_eq!(std::fs::read_to_string(cached_path).unwrap(), "custom backup");
    }
    ...
    fn image_bytes_are_saved_to_custom_backup_directory() {
        ...
        let id = service.collect_image_bytes_snapshot(&test_png_bytes()).unwrap().unwrap();
        let image_meta = service.get_image_meta(&id).unwrap().unwrap();
    ...
    fn ocr_text_is_saved_as_image_note() {
        ...
        service.update_ocr_text(&image, "  识别出来的文字  ").unwrap();
