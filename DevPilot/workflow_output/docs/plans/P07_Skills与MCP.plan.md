...
| 批次 | Step | 目标 | 对应 FR | 状态 |
|---|---|---|---|---|
| B1 | S0 | 数据模型与迁移（L11） | FR-025/026/011 | - [x] |
| B2 | S1 | core-skills 解析/注册/启停 | FR-025 | - [x] |
| B2 | S2 | MCP 进程管理器 | FR-026 | - [x] |
| B3 | S3 | 技能生成器 + 导入导出 | FR-025 | - [x] |
| B3 | S4 | MCP 市场安装 | FR-010 | - [x] |
| B4 | S5 | 斜杠调用与输入框集成 | FR-025 | - [x] |
| B4 | S6 | MCP 管理页 UI | FR-026 | - [x] |
| B5 | S7 | 多模态输入（拖图+语音） | FR-011 | - [x] |
| B5 | S8 | 测试 | 全部 AC | - [x] |
| B6 | S9 | 文档与索引 | 项目规范 | - [x] |
...
- [x] 所有 Rust/前端单测通过，用例名或注释带 AC 编号（AC-012/013/028/029）。
- [x] `scripts/check_all` 全绿。
- [ ] 人工测试方案：真实 MCP server（如 filesystem server）安装启停重启；语音听写真机；拖图真窗口。
- [x] 每个 FR-010/011/025/026 至少一个 Step 覆盖；每个 AC-012/013/028/029 有自动化或人工覆盖。
- [x] 依赖与并行化地图与 Step 字段一致；同批 `[P]` 文件无交集。
