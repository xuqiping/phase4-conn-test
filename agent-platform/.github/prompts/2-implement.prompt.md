...
- 参照 [file_structure.md](/agent-platform/workflow_output/docs/file_structure.md) 和工作区已有实现，保持风格一致。
- **不要改动**只作为参考的文件。
- 实现并运行计划中描述的单元测试。
- 每完成一个 chunk（测试通过），**建议我提交一次 commit**（commit 当存档点）。
- 看不懂自己写的代码，先加注释或简化——**never commit code you can't explain**。
- 判断本步是否产出通用能力/约定，是则提示我沉淀进 [AGENTS.md](/agent-platform/workflow_output/项目规范约束/AGENTS.md)。
