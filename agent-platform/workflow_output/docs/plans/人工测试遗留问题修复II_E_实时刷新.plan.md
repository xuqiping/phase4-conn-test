...
- [x] **E1：事件定义与发布点（后端）**（e97a00c；33/33。偏差：GROUP 事件按组员集逐人广播（selectMemberUserIds 新增）而非单 actor——联动表「组徽标+组页秒级」需全员收到；charge/refund/backstop 兼发 MEMBER）
...
- [x] **E2：/ws/events 端点（后端）**（b8206f7；5/5。ConcurrentWebSocketSessionDecorator 包装（并发 send 串行化+慢连接 5s/256KB 剔除）替代独立 pingManager——30s @Scheduled ping 保活+失败剔除；stats() 计数器）
...
- [x] **E3：监听推送（后端）**（360b350；2/2。@Async+@TransactionalEventListener(AFTER_COMMIT)——回滚不发/推送不拖计费；JSON 字段齐断言；序列化失败丢弃计数。AFTER_COMMIT 集成级验证靠 IT 全链路+人工项）
  - **目标**：事件→WS JSON 下行
  - **动作**：
    ```
...
- [x] **E4：前端 store 接入（徽标实时）**（197ca6b；vue-tsc 0。偏差：AppHeader 零改动（外层 WIP 文件不触碰）——生命周期自管理：init() 内启动、token 清空重连循环自终止、重登 AppHeader 重挂载再启动；token 主动刷新不重连（连接仅收推送无写面，低风险，下次断连用新 token）
...
- [x] **E5：页面刷新钩子（钱包/组页）**（870b0c9；vue-tsc 0。watch lastEvent 防抖 1s+visibilityState 不可见跳过；取简方案：不监听 visibilitychange 回前台补查——徽标 store 已新，列表下次事件/手动切查询）
  - **目标**：流水/列表跟随余额变
  - **动作**：
    ```
...
- [ ] **E6：铃铛轮询替换（可选，非验收项）** —— **未做（计划允许）**：铃铛保持 3s 轮询现状，收口记录于此；通道已就绪，后续加 notification 事件类型即可搭车
  - **目标**：3s 铃铛轮询搭同一通道
...
- [x] **E7：测试收口**（0fca445；单测 6+2+33、IT 12/12（真 PG，E1 组事件回归）、vue-tsc 0。轻量压测以单测级落地：5 用户×10 连接×100 轮=5000 帧无泄漏无丢弃（真实 50 连接+100 事件/秒压测属人工项，落测试方案）。规格 §3.5 人工四项待功能级收尾执行）
...
- [x] E1-E5/E7 全绿（E6 可选未做，已记录）；7x-3 单测面可勾销（人工四项待测试方案执行）；推送链路故障不影响计费（发布 catch-WARN+监听 catch-丢弃，单测锁死）
