...
| 2026-09-02 | **P4 运行验证完成**（冒烟 9fa0f25c + 文档本 commit）：smoke-xi.mjs 27/27（四功能核心链含反向）；**揪出并修 4 bug**——①组拖与画布平移叠加（d3 起手走伴生 mousedown，捕获段吞之）②selectOnly 只翻标志续会话（转正即终止会话）③键盘 Ctrl+Z 元素级焦点依赖死（并入 window 级 onWindowKeydown，删元素绑定防双弹）④撤销后 vue-flow 僵尸框选（applyHistoryState 补空参 removeSelectedElements()+getSnapshot 剥 selected，types 补字段）；+回归 4 用例（mock 补 removeSelectedElements），全量 1072/1072+vue-tsc 0 错；API 性能 p50/p95 全<40ms；问题单 2x+测试方案头部挂「P4 冒烟已过」+剩余人工项清单；速查表补修复XI 部署要点+变更行；开发进度5（含 vue-flow 自动化五坑沉淀） | P4 出口条件逐项过；plan D2 两处边界经真实拖动实证修正 |
| 2026-09-02 | **P4 交叉 review 收口**（第二 AI 审 fe22581c..HEAD 全量 diff）：8 发现——修 7（**高①** 后端分支1 整桶收 children→一级重命名丢仍存子类 link 误降通用，同分支2 过滤口径+AssetProjectServiceTest 红绿双验；**中①** 组↔组恰一端进板 `group:undefined` 断边→板内组 group:{复制索引}/板外保原组 id+aliveGroupIds 第 5 参；**中③** ✕ 解散悬挂组选→清 groupSelectedId；**中④** VocabEditor 撞名清空=normalizedRoles 静默删桶带子类→保留输入+rowError+canSave 门禁（AC-XI3-5 改新口径）；**低①** abort 按 top.tag 误弹→栈帧身份 {nodeId,entry}，自测再揪 abort 他人时误清记录一层；**低②** 官方库 size:100 截断→total 续页封顶 51 页；**低③** groups-pasted 节点数含散点→ΣmemberIds）+驳 1（中② official 不判 mode——publish 单写路径 admin 强制 OPEN+publishedByAdmin 双设，unpublish 双清，official⇒OPEN 写入端不变量已保证）；修后全量：后端 **2735/2735**+前端 **1077/1077**+vue-tsc 0；坑：MP IN 参数惰性物化（getParamNameValuePairs 须先 getSqlSegment 渲染） | 交叉 review 铁律 #6 第二 AI 兑现；「测试不是 review 替代品」实证——高①既有测试全绿下仍存活 |
