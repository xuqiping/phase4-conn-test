...
- （2026-08-18 P4 冒烟发现）访问不存在的后端路径（如打错 URL）返回 500「未预期异常」+ ERROR 级日志，而非 404——根因：`GlobalExceptionHandler` 未映射 Spring 的 `NoResourceFoundException`（Boot 3.2 静态资源解析抛），落入兜底 500。影响：错误分类失真 + 无谓 ERROR 噪音污染风险大盘/告警。修法：handler 加 `@ExceptionHandler(NoResourceFoundException.class)` → 404 WARN。低优先（功能无损，仅状态码与日志级别）。
